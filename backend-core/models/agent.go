...
// Scoped per workspace: agent names repeat across tenants, so every lookup must
// filter on WorkspaceID or one tenant can read (or kill) another tenant's agent.
