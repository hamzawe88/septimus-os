...
1. **62 unchecked type assertions** — `c.Locals("user_id").(string)` / `("workspace_id").(string)` across handlers. If the JWT middleware is ever missing/misordered on a route, these **panic** (crash the request goroutine). Use the comma-ok form: `v, _ := c.Locals("user_id").(string)`.

2. **Anthropic & Ollama providers are UI-only** — `AISettings.tsx` lets you enter a key and "Set as Active" for Anthropic/Ollama, but `ai-sidecar/providers.py` only implements `openai` + `gemini` (`# TODO: anthropic, ollama`). Selecting them → silent fallback to env/None → "no provider configured". Either implement them (`langchain-anthropic`, `langchain-ollama` — not yet in `requirements.txt`) or disable their cards.

3. **Dual AI config source** — legacy `AIConfig` table (`ConfigAI` handler) coexists with the real `ai_providers` workspace setting. The AI Center display was reading the stale table (now fixed to read `ai_providers`), but the table + handler still exist and can drift. Remove `AIConfig`/`ConfigAI` or make it a view over `ai_providers`.

### 🟡 MEDIUM

1. **Hardcoded default workspace `797ec9d1-…`** — appears in ~48 files (mostly frontend fallbacks + sidecar `DEFAULT_WORKSPACE_ID`). Fine for single-tenant demo, a correctness landmine for multi-tenant. Derive workspace from the authenticated session everywhere.

2. **Ghost service still in repo** — `ai-agents/` (marked `DEPRECATED.md`) is dead (broken import, not in compose, third vector store). Delete it to stop future confusion.

3. **Realtime voice not implemented** — Huddle STT→LLM→TTS path exists but the "realtime" experience (OpenAI Realtime API) is not built; needs a key + browser audio.

4. **i18n scan: 31 candidate unwired literals** (`python3 scripts/i18n_scan.py`). Most are false positives (currency codes `LYD/USD/EUR`, timezone IDs `Africa/Tripoli`); a handful are real (`"Enter"`, some mock-data words in widgets). Run the scanner before shipping UI text.

### 🟢 LOW / hygiene

1. **No automated tests** — no Go tests, no pytest, no frontend tests. All verification is manual/live. A minimal test suite + CI would catch regressions cheaply.
2. **Logging is `print`/`log.Printf` only** — no structured logging, no token-cost/observability export (Langfuse/OTel). Feedback rollup exists (`/reports/ai`) but no per-call tracing.
3. **`README.md`** predates the AI overhaul — update the "getting started" + architecture section.
