...
The overlay also starts the official version-matched `n8n-task-runners`
sidecar. Set a dedicated `N8N_RUNNERS_AUTH_TOKEN` in `.env` before startup
(`openssl rand -hex 32` is suitable). JavaScript and native Python Code nodes
run in that isolated container, not in the main n8n process.
...
2. For production, set a strong unique `N8N_ENCRYPTION_KEY` in `.env`
   **before** the first boot, back it up securely, and never rotate it in place.
   For an existing local volume where the variable was never set, leave it
   unset: n8n will reuse the key persisted in `/home/node/.n8n/config`.

The Compose overlay deliberately has no fallback encryption key. A fallback can
silently differ from the key already stored in the volume, causing n8n to enter
a restart loop and making saved credentials unavailable. If a production key is
lost, restore both the key and the matching n8n data backup together.
