...
export default function SaaSPlansManager() {
  ...
  return (
    <div className="space-y-8 pb-12 text-foreground dark:text-slate-100 w-full" dir={isRtl ? "rtl" : "ltr"}>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white/50 dark:bg-slate-900/50 rounded-[2rem] border border-dashed border-border dark:border-slate-700">
          <ShieldCheck className="w-16 h-16 text-slate-300 dark:text-muted-foreground mb-4" />
              ...
              >
                ...
                <div className="mb-6 mt-2">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold tracking-widest text-muted-foreground uppercase">{plan.tier_id}</span>
                  </div>
                  <h3 className="text-2xl font-black mb-2 text-foreground dark:text-white">
                    {isRtl ? plan.name_ar : plan.name_en}
                  </h3>
                  <p className="text-sm text-muted-foreground dark:text-muted-foreground min-h-[40px] leading-relaxed">
                    {isRtl ? plan.description_ar : plan.description_en}
                  </p>
                </div>

                <div className="mb-8 p-4 bg-muted dark:bg-slate-950/50 rounded-2xl border border-border dark:border-slate-800/50">
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-black tracking-tight text-foreground dark:text-white">
                      {plan.price === 0 ? (isRtl ? "مجاني" : "Free") : plan.price}
                    </span>
                    {plan.price > 0 && (
                      <span className="text-muted-foreground font-bold">{plan.currency}</span>
                    )}
                  </div>
                ...
                <div className="space-y-3 mb-8 flex-1">
                  ...
                  {(isRtl ? parsedFeaturesAr : parsedFeaturesEn).map((feat, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-foreground dark:text-slate-300 font-medium leading-relaxed">{feat}</span>
                    </div>
                  ))}
                </div>

                <div className="flex items-center gap-3 pt-6 border-t border-border dark:border-slate-800">
                  <Button 
                    onClick={() => handleOpenModal(plan)} 
                    variant="outline" 
                    className="flex-1 rounded-xl h-11 font-bold border-border dark:border-slate-700 hover:bg-muted dark:hover:bg-slate-800 text-muted-foreground dark:text-slate-300"
                  >
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-3xl p-0 overflow-hidden bg-card dark:bg-slate-950 border-border dark:border-slate-800 shadow-2xl rounded-[2rem]" dir={isRtl ? "rtl" : "ltr"}>
          <DialogHeader className="p-8 md:p-10 pb-0">
            <DialogTitle className="text-2xl font-black text-foreground dark:text-white flex items-center gap-3">
              <Sparkles className="w-6 h-6 text-brand" />
              {editingPlan ? (isRtl ? "تعديل الباقة" : "Edit Plan") : (isRtl ? "إضافة باقة جديدة" : "Add New Plan")}
            </DialogTitle>
            <DialogDescription className="text-muted-foreground dark:text-muted-foreground mt-2 text-base">
          ...
          <form onSubmit={handleSave} className="p-8 md:p-10 space-y-10 overflow-y-auto max-h-[70vh]">
            {/* Toggles */}
            <div className="flex flex-wrap items-center gap-8 p-6 bg-muted dark:bg-slate-900/50 rounded-2xl border border-border dark:border-slate-800">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className={`w-14 h-7 rounded-full transition-colors relative flex items-center shrink-0 ${formData.is_active ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}>
                  ...
                  <div className={`w-5 h-5 bg-card rounded-full shadow-sm absolute transition-all ${formData.is_active ? (isRtl ? 'start-1' : 'start-8') : (isRtl ? 'start-8' : 'start-1')}`} />
                </div>
                <span className="text-base font-bold text-foreground dark:text-slate-300 group-hover:text-emerald-500 transition-colors">
              ...
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className={`w-14 h-7 rounded-full transition-colors relative flex items-center shrink-0 ${formData.recommended ? 'bg-brand' : 'bg-slate-300 dark:bg-slate-700'}`}>
                  ...
                  <div className={`w-5 h-5 bg-card rounded-full shadow-sm absolute transition-all ${formData.recommended ? (isRtl ? 'start-1' : 'start-8') : (isRtl ? 'start-8' : 'start-1')}`} />
                </div>
                <span className="text-base font-bold text-foreground dark:text-slate-300 group-hover:text-brand transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">{t('admin.planTier') || "الرمز (TIER ID)"}</label>
                <input required value={formData.tier_id} onChange={e => setFormData({...formData, tier_id: e.target.value})} placeholder="e.g. starter, pro, enterprise" className="w-full px-5 h-14 bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-xl text-base font-medium focus:ring-2 focus:ring-brand focus:border-transparent outline-none transition-all shadow-sm" />
              </div>

              <div className="space-y-3">
                <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">{t('admin.planColor') || "Color Theme"}</label>
              ...
              <div className="space-y-3">
                <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">{isRtl ? "الاسم" : "Name"}</label>
                <input required value={isRtl ? formData.name_ar : formData.name_en} onChange={e => setFormData(isRtl ? {...formData, name_ar: e.target.value} : {...formData, name_en: e.target.value})} className="w-full px-5 h-14 bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-xl text-lg font-bold focus:ring-2 focus:ring-brand focus:border-transparent outline-none transition-all shadow-sm" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">{t('admin.planPrice') || "السعر"}</label>
                  <input required type="number" step="0.01" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} className="w-full px-5 h-14 bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-xl text-xl font-black focus:ring-2 focus:ring-brand focus:border-transparent outline-none transition-all shadow-sm" />
                </div>
                <div className="space-y-3">
                  <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">{t('admin.planCurrency') || "العملة"}</label>
                  <input required value={formData.currency} onChange={e => setFormData({...formData, currency: e.target.value})} className="w-full px-5 h-14 bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-xl text-base font-bold uppercase focus:ring-2 focus:ring-brand focus:border-transparent outline-none transition-all shadow-sm" />
                </div>
              </div>
            </div>

            <div className="space-y-3">
              <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">{isRtl ? "الوصف" : "Description"}</label>
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-foreground dark:text-slate-300 uppercase tracking-wider">
                {isRtl ? "المميزات المضمنة" : "Included Features"}
              </label>
              
              <div className="space-y-3">
                {featuresList.map((feat, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-muted dark:bg-slate-900/50 border border-border dark:border-slate-800 p-3 rounded-xl group transition-all hover:shadow-sm">
                    <div className="w-8 h-8 rounded-full bg-brand/10 text-brand flex items-center justify-center shrink-0">
                      <Check className="w-4 h-4" />
                    </div>
                    <span className="flex-1 text-sm font-medium text-foreground dark:text-slate-300">{feat}</span>
                    <button 
                      type="button" 
                      onClick={() => setFeaturesList(featuresList.filter((_, i) => i !== idx))}
                      className="w-8 h-8 rounded-full hover:bg-rose-100 dark:hover:bg-rose-500/20 text-muted-foreground hover:text-rose-500 flex items-center justify-center transition-colors md:opacity-0 group-hover:opacity-100"
                ...
                {featuresList.length === 0 && (
                  <div className="text-center py-6 bg-muted/50 dark:bg-slate-900/20 border border-dashed border-border dark:border-slate-800 rounded-xl text-muted-foreground text-sm">
                ...
                <div className="flex gap-3 pt-2">
                  <input 
                    ...
                    className="flex-1 px-4 h-12 bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-xl text-sm focus:ring-2 focus:ring-brand focus:border-transparent outline-none transition-all shadow-sm"
                  />
                  <Button 
                    type="button"
                    onClick={() => {
                    ...
                    className="h-12 px-6 bg-slate-900 hover:bg-slate-800 dark:bg-card dark:hover:bg-muted text-white dark:text-foreground font-bold rounded-xl shadow-md transition-all"
                  >
            ...
            <div className="pt-8 border-t border-border dark:border-slate-800 space-y-6">
              <div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl border transition-colors text-start ${
                        on
                          ? "border-brand bg-brand/5 text-foreground dark:text-white"
                          : "border-border dark:border-slate-700 text-muted-foreground dark:text-muted-foreground"
                      }`}
                    >
                      ...
                      <span className={`w-10 h-5 rounded-full relative shrink-0 transition-colors ${on ? "bg-brand" : "bg-slate-300 dark:bg-slate-700"}`}>
                        <span className={`w-4 h-4 bg-card rounded-full shadow absolute top-0.5 transition-all ${on ? "start-5" : "start-0.5"}`} />
                      </span>
                    </button>
                  );
                })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {(catalog?.limit_keys || []).map((key) => (
                  <div key={key}>
                    <label htmlFor={`limit-${key}`} className="block text-xs font-semibold text-muted-foreground dark:text-muted-foreground mb-1.5 font-mono">
                      {key}
                    </label>
                    <input
                      id={`limit-${key}`}
                      ...
                      className="w-full h-11 px-3 rounded-lg border border-border dark:border-slate-700 bg-card dark:bg-slate-900 text-foreground dark:text-white"
                    />
                  </div>
                ))}
              </div>
              <p className="text-xs text-muted-foreground">{isRtl ? "استخدم القيمة -1 لغير محدود." : "Use -1 for unlimited."}</p>
            </div>

            <div className="pt-8 border-t border-border dark:border-slate-800 flex justify-end gap-4">
              <Button type="button" variant="ghost" onClick={() => setIsModalOpen(false)} className="h-14 px-8 text-base font-bold rounded-xl hover:bg-muted dark:hover:bg-slate-800">
                {isRtl ? "إلغاء" : "Cancel"}
              </Button>
