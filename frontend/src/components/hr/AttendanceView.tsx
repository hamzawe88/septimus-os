...
export default function AttendanceView() {
  ...
  const fetchRecordsAndEmployees = useCallback(async () => {
    ...
    try {
      ...
      const empRes = await fetchWithAuth(`${API_BASE_URL}/entities?workspace_id=${workspaceId}&type=hr_employee&limit=50`);
      if (empRes.ok) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] w-full overflow-hidden">
      ...
      {/* Header */}
      <div className="flex-none px-8 py-6 border-b border-slate-200 bg-white shadow-sm">
        <div className="flex justify-between items-start">
          ...
          <div className="text-end flex flex-col items-end gap-2">
            <div className="flex items-center gap-2 text-slate-800 font-mono text-xl font-black bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 shadow-inner">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all flex items-center gap-1.5 shadow-sm ${
                  showSettings ? "bg-indigo-600 text-white border-indigo-600 ring-2 ring-indigo-200" : "bg-white hover:bg-slate-50 text-slate-700 border-slate-300"
                }`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          <div className="bg-emerald-50/80 border border-emerald-200 rounded-2xl p-4 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-600 font-bold">{t("hr.presentToday") || "الحاضرون اليوم"}</p>
          ...
          <div className={`border rounded-2xl p-4 flex items-center justify-between shadow-sm transition-colors ${lateCount > 0 ? "bg-amber-50/80 border-amber-300" : "bg-white border-slate-200"}`}>
            <div className="flex items-center gap-3">
              <AlertTriangle className={`w-10 h-10 rounded-xl p-2 shadow-inner ${lateCount > 0 ? "text-amber-700 bg-amber-100" : "text-slate-400 bg-slate-100"}`} />
              <div>
                <p className="text-xs text-slate-600 font-bold">{t("hr.lateEmployees") || "الموظفون المتأخرون"}</p>
                <p className={`text-3xl font-black mt-0.5 ${lateCount > 0 ? "text-amber-700" : "text-slate-800"}`}>{lateCount}</p>
              </div>
            </div>
            <span className={`text-xs font-extrabold px-2.5 py-1 rounded-full border ${lateCount > 0 ? "bg-amber-100 text-amber-800 border-amber-300" : "bg-slate-100 text-slate-500 border-slate-200"}`}>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-600 font-bold">{t("hr.totalRecords") || "إجمالي السجلّات المعتمدة"}</p>
                <p className="text-3xl font-black text-slate-800 mt-0.5">{todayRecords.length}</p>
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-extrabold text-slate-800 mb-4 flex items-center justify-between">
              ...
              <span className="text-xs bg-slate-100 text-slate-600 font-mono font-bold px-2.5 py-1 rounded-lg border border-slate-200">
            ...
            <div className={`rounded-2xl p-5 mb-4 text-center border-2 transition-all shadow-sm ${
              ...
              {geoStatus === "idle" && (
                <>
                  ...
                  <p className="text-sm font-bold text-slate-600">{t("hr.clickToLocate") || "انقر أدناه لتحديد واختبار موقعك الحالي"}</p>
                </>
              )}
              {geoStatus === "locating" && (
                <>
            ...
            <div className="pt-3 border-t border-slate-100">
              <p className="text-xs font-bold text-slate-600 mb-2.5 flex items-center justify-between">
                <span className="flex items-center gap-1.5 text-indigo-700">
                  <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                  {t("hr.gpsSimulator") || "محاكاة الموقع للاختبار الفوري"}:
                </span>
                <span className="text-[11px] text-slate-400 font-normal">{t("hr.simulationOnly") || "(لتجربة أزرار الحضور دون مشي)"}</span>
              </p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-3 text-xs font-bold text-slate-400 uppercase">
              ...
              <div>
                <label htmlFor="emp_name" className="block text-xs font-bold text-slate-700 mb-1.5">
                ...
                {employees.length > 0 && (
                  <div className="mb-2.5">
                    <select
                      ...
                      className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-slate-800 focus:border-indigo-500 outline-none"
                    >
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-slate-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none"
                />
              </div>
        ...
        <div className="lg:col-span-7">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden h-full flex flex-col">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="text-xs font-bold border border-slate-300 rounded-xl px-3 py-2 text-slate-700 focus:border-indigo-500 outline-none bg-white shadow-sm"
                />
                <button
                  type="button"
                  onClick={fetchRecordsAndEmployees}
                  className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl border border-slate-200 hover:border-indigo-200 transition-all shadow-sm"
            ...
            {loading && records.length === 0 ? (
              <div className="p-16 text-center text-slate-400 text-sm flex flex-col items-center justify-center flex-1">
                <RefreshCw className="w-8 h-8 text-indigo-500 animate-spin mb-3" />
                <p className="font-bold">{t("common.loading") || "جارٍ تحميل سجلات الحضور من قاعدة البيانات..."}</p>
              </div>
            ) : todayRecords.length === 0 ? (
              <div className="p-16 text-center flex flex-col items-center justify-center flex-1 bg-slate-50/30">
                <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center mb-4 text-slate-400">
                  <Users className="w-8 h-8" />
                </div>
                <p className="text-slate-600 font-bold text-base">{t("hr.noAttendanceRecords") || "لا توجد سجلّات حضور مسجلة لهذا اليوم"}</p>
                <p className="text-slate-400 text-xs mt-1 font-medium">{t("hr.startCheckIn") || "انقر على «تسجيل الحضور الفوري لحسابي الآن» أو زر المحاكاة للبدء الفوري"}</p>
              </div>
            ) : (
                ...
                {todayRecords.map((record) => (
                  <div key={record.id} className="px-6 py-4.5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-indigo-50/30 transition-all">
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        <p className="text-sm font-extrabold text-slate-900">{record.employee_name}</p>
                        <div className="flex flex-wrap items-center gap-3 mt-1">
                          <span className="text-xs font-bold text-slate-600 flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
                            <LogIn className="w-3.5 h-3.5 text-emerald-600" />
                            {t("hr.checkIn") || "حضور"}: {formatTime(record.check_in, isRtl)}
                          </span>
                          {record.check_out && (
                            <span className="text-xs font-bold text-slate-600 flex items-center gap-1 bg-rose-50 px-2 py-0.5 rounded-md border border-rose-200">
                    ...
                    <div className="flex items-center gap-3 self-end sm:self-center">
                      ...
                      {record.status === "checked_out" && <span className="px-3 py-1 text-xs font-extrabold rounded-full bg-slate-100 text-slate-700 border border-slate-300">✓ {t("hr.checkedOut") || "انصرف"}</span>}
