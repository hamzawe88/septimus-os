  ...
  useEffect(() => {
    const fetchOrgData = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error('فشل في جلب بيانات الهيكل التنظيمي');
        }
    ...
    return (
      <div className="w-full h-full min-h-[500px] flex items-center justify-center bg-slate-50 rounded-xl border border-[var(--border-strong)]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[var(--text-secondary)]">جاري بناء الشبكة...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full min-h-[600px] gap-4">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <h3 className="font-bold text-slate-800">مستكشف الهيكل التنظيمي</h3>
        <div className="flex gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-slate-700">عرض الأقسام</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              type="checkbox" 
            ...
            />
            <span className="text-sm font-medium text-slate-700">عرض الموظفين</span>
          </label>
        </div>
      </div>
          ...
          <div className="w-1/3 bg-white border border-slate-200 rounded-xl shadow-sm p-6 flex flex-col animate-in slide-in-from-left-8 h-[600px] overflow-y-auto">
            <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  {selectedNode.group === 'department' ? 'إدارة / قسم' : 'موظف'}
                </p>
              </div>
              <button 
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                <p className="text-xs text-slate-500 font-medium mb-1">المعرف (ID)</p>
                <p className="text-sm text-slate-700 font-mono break-all">{selectedNode.id}</p>
              </div>
              
              {selectedNode.role && (
                <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                  <p className="text-xs text-slate-500 font-medium mb-1">الدور الوظيفي</p>
              ...
              {selectedNode.group === 'user' && (
                <div className="bg-brand-light/50 p-4 rounded-lg border border-brand-light mt-2">
                  <p className="text-xs text-brand font-medium mb-2">معلومات إضافية (مستقبلاً)</p>
                  <ul className="text-sm text-slate-600 list-disc list-inside space-y-1">
                    <li>المشاريع المرتبطة</li>
                    <li>الأقسام المتعددة</li>
                    <li>فريق العمل المشترك</li>
                  </ul>
                </div>
              )}
            </div>
