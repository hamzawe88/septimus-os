...
export default function OrgChartContainer() {
  ...
  if (loading) {
    return (
      <div className="w-full h-full min-h-[500px] flex items-center justify-center bg-muted rounded-xl border border-[var(--border-strong)]">
  ...
  return (
    <div className="flex flex-col h-full min-h-[600px] gap-4">
      ...
      <div className="flex justify-between items-center bg-card p-4 rounded-xl border border-border shadow-sm">
        <h3 className="font-bold text-foreground">{isRtl ? "مستكشف الهيكل التنظيمي" : "Org Chart Explorer"}</h3>
        <div className="flex gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-foreground">{isRtl ? "عرض الأقسام" : "Show Departments"}</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              type="checkbox" 
            ...
            />
            <span className="text-sm font-medium text-foreground">{isRtl ? "عرض الموظفين" : "Show Employees"}</span>
          </label>
        </div>
      </div>
      ...
      <div className="flex flex-1 gap-4 relative overflow-hidden">
        ...
        {/* Side Panel */}
        {selectedNode && (
          <div className="w-1/3 bg-card border border-border rounded-xl shadow-sm p-6 flex flex-col animate-in slide-in-from-left-8 h-[600px] overflow-y-auto">
            <div className="flex justify-between items-start mb-6 border-b border-border pb-4">
              ...
              <button 
                ...
                className="text-muted-foreground hover:text-muted-foreground bg-muted hover:bg-slate-200 p-2 rounded-full transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-muted p-4 rounded-lg border border-border">
                <p className="text-xs text-muted-foreground font-medium mb-1">{isRtl ? "المعرف" : "ID"}</p>
                <p className="text-sm text-foreground font-mono break-all">{selectedNode.id}</p>
              </div>
              
              {selectedNode.role && (
                <div className="bg-muted p-4 rounded-lg border border-border">
                  <p className="text-xs text-muted-foreground font-medium mb-1">{isRtl ? "الدور الوظيفي" : "Role"}</p>
                  <p className="text-sm text-foreground">{selectedNode.role}</p>
                </div>
              )}
