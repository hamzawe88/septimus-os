...
export default function FinanceInvoicesView({ entities, loading, onNewInvoice, onRefresh }: FinanceInvoicesViewProps) {
  ...
  return (
    <div className="flex flex-col h-full text-foreground">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-muted rounded-lg p-1 border border-border">
            <button
              ...
              className={`p-1.5 rounded-md transition-colors ${viewMode === "kanban" ? "bg-card shadow-sm text-brand" : "text-muted-foreground hover:text-foreground"}`}
            ...
            <button
              ...
              className={`p-1.5 rounded-md transition-colors ${viewMode === "table" ? "bg-card shadow-sm text-brand" : "text-muted-foreground hover:text-foreground"}`}
      ...
      {/* Search & Filter */}
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm mb-6 flex items-center">
        ...
        <input
          ...
          className="bg-transparent border-none outline-none text-foreground w-full placeholder:text-muted-foreground text-sm"
        />
      </div>
      ...
      <div className="flex-1 min-h-0">
        {loading ? (
          <div className="flex items-center justify-center h-full text-muted-foreground">{t("finance.loadingInvoices")}</div>
        ) : filteredEntities.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground border-2 border-dashed border-border rounded-2xl bg-white/50 p-12 text-center">
            <FileText className="w-12 h-12 text-slate-300 mb-3" />
            <p className="font-bold text-foreground">{t("finance.noInvoicesFound")}</p>
            <p className="text-xs text-muted-foreground mt-1">{t("finance.noInvoicesDesc")}</p>
          </div>
        ) : viewMode === "table" ? (
          <div className="bg-card rounded-xl border border-border overflow-hidden h-full overflow-y-auto shadow-sm">
            <table className="w-full text-start">
              <thead className="bg-muted border-b border-border sticky top-0 text-xs font-bold text-muted-foreground uppercase">
                <tr>
              ...
              <tbody className="divide-y divide-border text-sm">
                {filteredEntities.map((entity) => {
                  ...
                  return (
                    <tr key={String(entity.id)} className="hover:bg-muted transition-colors">
                      <td className="p-4 font-mono font-bold text-foreground cursor-pointer" onClick={() => setSelectedInvoice(entity)}>{num}</td>
                      <td className="p-4 cursor-pointer" onClick={() => setSelectedInvoice(entity)}>
                        <p className="font-bold text-foreground">{client}</p>
                        {comp && <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5"><Building2 className="w-3 h-3 text-brand" />{comp}</p>}
                      </td>
                      ...
                      <td className="p-4 font-mono text-muted-foreground cursor-pointer" onClick={() => setSelectedInvoice(entity)}>{due}</td>
                      <td className="p-4 cursor-pointer" onClick={() => setSelectedInvoice(entity)}>
                      ...
                      <td className="p-4 text-xs text-muted-foreground font-mono cursor-pointer" onClick={() => setSelectedInvoice(entity)}>
          ...
          <DragDropContext onDragEnd={handleDragEnd}>
            <div className={`flex h-full space-x-6 overflow-x-auto pb-4 ${isRtl ? "space-x-reverse" : ""}`}>
              {BASE_COLUMNS.map((col) => {
                ...
                return (
                  <div key={col.id} className="w-80 flex flex-col h-full bg-muted/80 rounded-2xl border border-border/80 shrink-0 shadow-sm overflow-hidden">
                    <div className={`flex items-center justify-between p-4 border-b border-border bg-card ${col.bg}`}>
                      <h3 className="font-bold text-foreground text-sm">{t(col.titleKey, col.defaultTitle)}</h3>
                      <span className="text-xs font-bold bg-card text-foreground py-1 px-2.5 rounded-full border border-border shadow-2xs">
                              ...
                              <Draggable key={invoice.id} draggableId={invoice.id} index={index}>
                                {(dragProvided) => (
                                  <div
                                    ...
                                    className="bg-card p-4 rounded-xl shadow-sm border border-border cursor-pointer hover:border-brand hover:shadow-md transition-all group relative"
                                    onClick={() => setSelectedInvoice(invoice)}
                                  >
                                    <div className="flex items-center justify-between mb-2">
                                      <span className="font-mono text-xs font-bold px-2 py-0.5 rounded bg-muted text-foreground border border-border">{num}</span>
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                    ...
                                    <div className="font-bold text-foreground text-sm mb-0.5">{client}</div>
                                    {comp && <div className="text-xs text-muted-foreground font-medium flex items-center gap-1 mb-2"><Building2 className="w-3 h-3 text-brand" />{comp}</div>}
                                    <div className="text-xs text-muted-foreground mb-3 font-mono">{t("finance.dueOn")} {due}</div>
                                    <div className="flex justify-between items-center pt-2 border-t border-border">
                                      <span className="text-brand font-bold font-mono text-base">{formatCurrency(amt)}</span>
                                      <span className="text-[11px] text-muted-foreground font-mono">{new Date(String(invoice.created_at)).toLocaleDateString()}</span>
                                    </div>
                                  </div>
                                )}
                              </Draggable>
