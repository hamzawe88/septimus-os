        ...
        <div className="bg-white dark:bg-[#1a1d21] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden mb-8">
              ...
              {!generatedKey ? (
                <>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <Button variant="ghost" size="sm" className="text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-800 dark:hover:text-white" onClick={() => setIsAdding(false)}>
                      {t("common.cancel")}
                    </Button>
                  </div>
                  <div className="mb-4">
                ...
                <div className="bg-green-50 dark:bg-green-950/40 p-6 rounded-lg border border-green-200 dark:border-green-800">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <input 
                      ...
                      readOnly 
                      value={generatedKey} 
                      className="flex-1 p-3 bg-white dark:bg-[#1a1d21] border border-green-300 dark:border-green-700 text-slate-900 dark:text-white rounded-md font-mono text-sm"
                    />
                    <Button aria-label="Copy to clipboard" onClick={handleCopyKey} className="bg-white dark:bg-[#1a1d21] border border-green-300 dark:border-green-700 text-green-700 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30 h-full py-3">
                      {copied ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                    </Button>
                  </div>
                  <div className="flex justify-end">
                    <Button onClick={() => { setIsAdding(false); setGeneratedKey(""); }} variant="outline" className="text-slate-700 dark:text-slate-300 border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 bg-white dark:bg-[#1a1d21]">
                      {t("common.done")}
                    </Button>
                  </div>
                </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {loading ? (
              ...
              keys.map(key => (
                <div key={key.ID} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-[#f8fafc] dark:hover:bg-[#222529] transition-colors">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className={`w-2 h-2 rounded-full ${key.IsActive ? 'bg-green-500' : 'bg-red-500'}`}></span>
                      <h3 className="font-semibold text-slate-800 dark:text-white">{key.Name}</h3>
                    </div>
                    <div className="flex items-center gap-2 mt-2 ms-5 text-sm text-slate-500 dark:text-slate-400 font-mono">
                      <Key className="w-4 h-4" />
                      {key.KeyHash.substring(0, 8)}... (Hash)
                    </div>
                    <div className="ms-5 mt-1 text-xs text-slate-400">
