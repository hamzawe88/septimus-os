        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-8 flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all ${
                    activeCategory === cat
                      ...
                      : "bg-muted dark:bg-slate-800 text-muted-foreground dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"
                  }`}
                >
                  {catLabel(cat)}
                </button>
            ...
            <div className="relative w-full md:w-80">
              <div className="absolute top-1/2 -translate-y-1/2 start-3 text-muted-foreground">
              ...
              <input
                ...
                className="w-full ps-10 pe-4 py-2.5 bg-card dark:bg-[#222529] border border-border dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand/50 text-foreground dark:text-slate-200"
              />
            </div>
          </div>
        </div>
      ...
      <div className="p-8 max-w-7xl mx-auto w-full">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
            ...
            <h3 className="text-xl font-bold mb-2 text-muted-foreground dark:text-slate-300">{loadError}</h3>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div className="bg-card dark:bg-[#1a1d21] border border-amber-300 dark:border-amber-700/50 rounded-2xl p-6 flex flex-col hover:shadow-xl hover:border-amber-500/50 transition-all group shadow-md shadow-amber-500/5 ring-1 ring-amber-400/20">
              ...
              <div className="mb-6 flex-1">
                <h3 className="text-lg font-bold text-foreground dark:text-white mb-1 group-hover:text-brand transition-colors">
                ...
                <p className="text-sm text-muted-foreground dark:text-slate-300 leading-relaxed line-clamp-3">
                  {isRtl
            ...
            {filtered.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-card dark:bg-[#1a1d21] border border-border dark:border-slate-800 rounded-2xl p-6 flex flex-col hover:shadow-xl hover:border-brand/30 transition-all group"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-14 h-14 bg-muted dark:bg-[#222529] rounded-2xl flex items-center justify-center text-brand border border-border dark:border-slate-800 group-hover:scale-110 transition-transform">
                  ...
                  <h3 className="text-lg font-black text-foreground dark:text-white mb-2">{integrationName(item, isRtl)}</h3>

                  <p className="text-sm text-muted-foreground dark:text-muted-foreground mb-6 flex-1 line-clamp-3">
                    {integrationDescription(item, isRtl)}
                  </p>

                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-border dark:border-slate-800">
                    <span className="text-xs font-medium text-muted-foreground">{catLabel(item.category)}</span>

                    {isConnected ? (
                      <button
                        onClick={() => setSelectedApp(item)}
                        className="flex items-center gap-2 text-sm font-bold text-muted-foreground dark:text-slate-300 hover:text-brand dark:hover:text-brand transition-colors bg-muted hover:bg-muted dark:bg-slate-800 dark:hover:bg-slate-700 px-4 py-2 rounded-xl"
                      >
