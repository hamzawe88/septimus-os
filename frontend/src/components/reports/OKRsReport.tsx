...
export default function OKRsReport() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-64 text-muted-foreground dark:text-muted-foreground">{t("reports.okrs.loading")}</div>;
  }
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return "text-muted-foreground bg-muted dark:text-muted-foreground dark:bg-slate-500/10";
    }
  };
    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-card dark:bg-[#1a1a1a] p-6 rounded-xl border border-border dark:border-slate-800 shadow-sm flex items-start gap-4 transition-colors">
          ...
          <div>
            <p className="text-sm text-muted-foreground dark:text-muted-foreground font-medium">{t("reports.okrs.totalObjectives")}</p>
            <h3 className="text-2xl font-bold text-foreground dark:text-slate-100 mt-1">{objectives.length}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card dark:bg-[#1a1a1a] rounded-xl border border-border dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
        <div className="px-6 py-4 border-b border-border dark:border-slate-800 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-foreground dark:text-slate-100">{t("reports.okrs.title")}</h3>
        </div>
        <div className="p-6 flex flex-col gap-6">
          {objectives.map(obj => (
            <div key={obj.id} className="border border-border dark:border-slate-800 rounded-xl p-6 bg-muted/50 dark:bg-[#121212] transition-colors">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                <div>
                  <h4 className="text-xl font-bold text-foreground dark:text-slate-100">{obj.title}</h4>
                  <p className="text-sm text-muted-foreground dark:text-muted-foreground mt-1">{t("reports.okrs.owner")} {obj.owner}</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-end">
                    <span className="text-2xl font-bold text-foreground dark:text-slate-100">{obj.progress}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h5 className="text-sm font-semibold text-foreground dark:text-slate-300 uppercase tracking-wider">{t("reports.okrs.keyResults")}</h5>
                {obj.key_results.map(kr => {
                  const krProgress = Math.min(100, Math.round((kr.current_value / kr.target_value) * 100));
                  return (
                    <div key={kr.id} className="bg-card dark:bg-[#1a1a1a] p-4 rounded-lg border border-border dark:border-slate-800 shadow-sm transition-colors">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-foreground dark:text-slate-100 font-medium">{kr.title}</span>
                        <span className="text-sm text-muted-foreground dark:text-muted-foreground">{kr.current_value} / {kr.target_value} {kr.unit} ({krProgress}%)</span>
                      </div>
                      <progress 
                        value={krProgress} 
                        max="100" 
                        className="w-full h-2 rounded-full [&::-webkit-progress-bar]:bg-muted dark:[&::-webkit-progress-bar]:bg-slate-800 [&::-moz-progress-bar]:rounded-full [&::-webkit-progress-value]:rounded-full [&::-webkit-progress-value]:bg-emerald-500 [&::-moz-progress-bar]:bg-emerald-500 text-emerald-500"
                      />
                    </div>
                  );
                })}
          ...
          {objectives.length === 0 && (
            <div className="text-center py-8 text-muted-foreground dark:text-muted-foreground">{t("reports.okrs.noObjectives")}</div>
          )}
        </div>
      </div>
    </div>
