...
const ColorInput = ({ label, value, onChange, placeholder }: { label: string, value: string, onChange: (v: string) => void, placeholder?: string }) => (
  <div className="group relative">
    <label className="block text-sm font-semibold text-foreground dark:text-slate-300 mb-2">{label}</label>
    <div className="relative flex items-center bg-white/50 dark:bg-[#1a1d21]/50 border border-border/60 dark:border-slate-700/60 rounded-xl overflow-hidden shadow-sm backdrop-blur-sm transition-all focus-within:ring-2 focus-within:ring-brand/30 focus-within:border-brand/50">
      <div className="relative w-12 h-12 shrink-0 border-r border-border/60 dark:border-slate-700/60 bg-muted dark:bg-[#222529]">
        <input 
          type="color" 
...
export default function AppearanceSettings() {
  ...
  const { 
    theme, setTheme, primaryColor, setPrimaryColor, fontFamily, setFontFamily, logoUrl, setLogoUrl,
      ...
      <div className="p-8 max-w-5xl mx-auto w-full space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <motion.section 
            ...
            className="bg-white/70 dark:bg-[#1a1d21]/70 backdrop-blur-xl p-8 rounded-[2rem] shadow-sm border border-border/60 dark:border-slate-800/60"
          >
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-amber-500/10 rounded-lg"><MonitorSmartphone className="w-5 h-5 text-amber-500" /></div>
              <div>
                <h2 className="text-lg font-bold text-foreground dark:text-white">{isRtl ? "نمط الإضاءة" : "Theme Mode"}</h2>
                <p className="text-sm text-muted-foreground dark:text-muted-foreground">{isRtl ? "اختر النمط المناسب لعينيك" : "Choose your preferred viewing mode"}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                { id: 'system', icon: Monitor, label: isRtl ? "تلقائي" : "System", desc: isRtl ? "حسب النظام" : "Follows system", color: "text-muted-foreground", bg: "bg-slate-500/10" }
              ].map(m => (
                <button
                  key={m.id}
                  onClick={() => setNextTheme(m.id)}
                  className={`relative p-4 rounded-2xl border transition-all flex items-center gap-4 group ${
                    nextTheme === m.id
                      ...
                      : 'border-border/60 dark:border-slate-700/60 hover:border-brand/30 hover:bg-muted/50 dark:hover:bg-[#222529]/50'
                  }`}
                >
                  ...
                  <div className="text-start flex-1">
                    <div className="font-bold text-foreground dark:text-white text-base">{m.label}</div>
                    <div className="text-sm text-muted-foreground dark:text-muted-foreground">{m.desc}</div>
                  </div>
                  {nextTheme === m.id && <CheckCircle2 className="w-5 h-5 text-brand" />}
                </button>
              ))}
            </div>
          </motion.section>
          ...
          <motion.section 
            ...
            className="bg-white/70 dark:bg-[#1a1d21]/70 backdrop-blur-xl p-8 rounded-[2rem] shadow-sm border border-border/60 dark:border-slate-800/60 flex flex-col"
          >
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-emerald-500/10 rounded-lg"><Settings className="w-5 h-5 text-emerald-500" /></div>
              <div>
                <h2 className="text-lg font-bold text-foreground dark:text-white">{isRtl ? "الخطوط والهوية" : "Typography & Logo"}</h2>
                <p className="text-sm text-muted-foreground dark:text-muted-foreground">{isRtl ? "تخصيص الخط والشعار" : "Customize font and branding"}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <button
                ...
                className={`font-cairo p-4 rounded-2xl border transition-all text-center flex flex-col items-center gap-2 ${
                  fontFamily === 'cairo' ? 'border-brand bg-brand/5 shadow-md' : 'border-border/60 dark:border-slate-700/60 hover:border-brand/30'
                }`}
              >
                <span className="text-3xl font-black text-foreground dark:text-white">Ag</span>
                <span className="font-bold text-sm text-foreground dark:text-slate-300">Cairo (Arabic)</span>
              </button>
              <button
                onClick={() => setFontFamily('inter')}
                className={`font-inter p-4 rounded-2xl border transition-all text-center flex flex-col items-center gap-2 ${
                  fontFamily === 'inter' ? 'border-brand bg-brand/5 shadow-md' : 'border-border/60 dark:border-slate-700/60 hover:border-brand/30'
                }`}
              >
                <span className="text-3xl font-black text-foreground dark:text-white">Ag</span>
                <span className="font-bold text-sm text-foreground dark:text-slate-300">Inter (English)</span>
              </button>
            </div>

            <div className="flex-1 flex flex-col justify-end">
              <label className="block text-sm font-semibold text-foreground dark:text-slate-300 mb-3">{isRtl ? "شعار مساحة العمل" : "Workspace Logo"}</label>
              <div className="flex items-center gap-5 p-4 bg-muted dark:bg-[#222529] rounded-2xl border border-border/60 dark:border-slate-700/60">
                <div className="w-16 h-16 bg-card dark:bg-[#121016] rounded-xl border border-border dark:border-slate-800 flex items-center justify-center overflow-hidden shadow-sm shrink-0">
                  {logoUrl ? (
                ...
                <div className="flex-1">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="flex items-center gap-2 px-4 py-2 bg-card dark:bg-[#1a1d21] text-foreground dark:text-slate-200 border border-border dark:border-slate-700 text-sm font-bold rounded-lg hover:border-brand hover:text-brand transition-all disabled:opacity-50"
                  >
        ...
        >
          <div className="flex items-start justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-foreground dark:text-white">{isRtl ? "الهوية اللونية" : "Color Identity"}</h2>
                <p className="text-sm text-muted-foreground dark:text-muted-foreground">{isRtl ? "تحكم كامل بألوان النظام" : "Full control over system colors"}</p>
              </div>
            </div>
            <label className="flex items-center gap-3 cursor-pointer group">
              <span className="text-sm font-bold text-muted-foreground dark:text-muted-foreground group-hover:text-foreground dark:group-hover:text-white transition-colors">
          ...
          <AnimatePresence mode="wait">
            {!isAdvancedMode ? (
                  ...
                  {(Object.keys(THEME_PRESETS) as ThemePreset[]).map((preset) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative flex flex-col items-center gap-3 rounded-[var(--radius-surface)] border p-4 transition-all ${
                          isActive 
                            ? 'border-brand bg-brand-light shadow-[var(--shadow-raised)]'
                            : 'border-border bg-card hover:border-brand/30 hover:bg-muted'
                        }`}
                      >
                        <div
                          className={`h-12 w-12 rounded-[var(--radius-control)] shadow-[var(--shadow-raised)] ring-4 ${isActive ? 'ring-brand/20' : 'ring-transparent group-hover:ring-brand/10'} transition-all`}
                          style={{ backgroundColor: presetData.primaryColor }}
                        />
                        <span className={`text-sm font-bold ${isActive ? 'text-brand' : 'text-foreground'}`}>
                          {t(presetData.labelKey)}
                        </span>
              ...
              <motion.div key="advanced" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 p-6 bg-muted dark:bg-[#121016]/50 rounded-[1.5rem] border border-border/60 dark:border-slate-700/60">
