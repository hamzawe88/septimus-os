...
const TaskComponent = (props: any) => {
  ...
  const createTask = async () => {
    ...
    // We'll just hardcode projectId for now since it's a demo
    const projectId = props.extension.options.projectId || "5bf90680-cf33-44ae-851c-eef563e82920"; 
  ...
  return (
    <NodeViewWrapper className="task-node group flex items-center gap-3 p-3 my-2 bg-white border border-slate-200 rounded-lg shadow-sm hover:border-brand-light transition-colors">
      <button contentEditable={false} onClick={toggleDone} disabled={loading} className="text-slate-400 hover:text-brand">
      ...
      <div contentEditable={false} className="flex items-center gap-2">
        {taskId ? (
          <span className="text-xs font-mono text-slate-400 bg-slate-100 px-2 py-1 rounded">T-{taskId.substring(0,4)}</span>
        ) : (
